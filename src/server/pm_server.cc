...
SingaServer::SingaServer(int id){
	...
	int fd = fopen(FLAGS_topology_config.c_str(),O_RDONLY);
	assert(fd);
	Topology topology;
	...
	for (int i=0; i<n_servers; i++){
		...
		if (server->id()==id_){
			sprintf(frontend_endpoint,"tcp://%s:%d",server->ip(),server->port());
			sprintf(backend_endpoint,"inproc://singanus:%d",server->ip(),server->port());
		}
		else{
			char *neighbor_endpoint = (char*)malloc(256);
			sprintf(neighbor_endpoint,"tcp://%s:%d",server->ip(),server->port());
			neighbors_[server->id()] = neighbor_endpoint;
		}
	}
}
...
void SingaServer::StartServer(){
	...
	for (map<int,char *>::iterator it = neighbors_.begin(); it!=neighbors_.end(); it++){
		void *socket = zsocket_new(context, ZMQ_DEALER);
		zsocket_connect(socket, it->second);
		neighbor_socket.push_back(socket);
	}

	//create ParamShard object to be shared between threads
	param_shard_ = new ParamShard(id_);

	//create and start server threads
	zthread_fork(ctx, ServerThread, this);
	...
	while (is_running){
		...
		for (int i=2; i<nsockets; i++)
			items[i] = {neighbor_socket[i-1], 0, ZMQ_POLLIN, 0};
...
void ServerThread(void *args, ztcx_t *ctx, void *pipe){
	...
	int rc = zsocket_connect(backend, backend_endpoint_);
	assert(rc==0);
	rc = zsocket_connect(frontend,frontend_endpoint_);
	...
	while (true){
		...
		// 	if equal to REQUEUED_ID -> pop another frame as the ID
		//	else it is the type
		...
		if (memcmp(next_frame, REQUEUE_ID, strlen(REQUEUE_ID))==0){
			...
			sscanf(zmsg_popstr(msg), "%d",&type);
		}
		else
			sscanf(next_frame,"%d",&type);

		sscanf(zmsg_popstr(msg),"%d",&paramId);
		zmsg_prepend(msg,&identity); //construct (<identity><content>) message
